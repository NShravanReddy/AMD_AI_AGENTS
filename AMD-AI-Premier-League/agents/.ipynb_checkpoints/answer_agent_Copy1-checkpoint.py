...
class AnsweringAgent(object):
    ...
    def build_prompt(self, question_data: Dict[str, str|Any]) -> Tuple[str, str]:
        ...
        sys_prompt1 = "You are an expert in solving competitive exam questions focused on logical reasoning and puzzle-based MCQs."
        "You specialize in the following areas:"
        "1. Puzzles involving generations and family tree logic" 
        "2. Seating Arrangements (Linear and Circular) " 
        "3. Truth-teller and Liar Problems"
        "For each question, apply domain-specific techniques: "
        "use diagrams where helpful, track relationships clearly, and deduce facts step-by-step."
        "Always explain your reasoning before selecting the correct option from [A, B, C, D]."
        "Be concise but precise in your logic."
        sys_prompt2 = (
    ...
    def answer_batches(self, questions: List[Dict], batch_size: int = 5, **kwargs) -> Tuple[List[Dict], List[int | None], List[float | None]]:
        ...
        for i in range(0, len(questions), batch_size):
            ...
            batch_sizes.append(len(batch_questions))
