...
class AnsweringAgent(object):
    ...
    def __init__(self, select_prompt1: bool = True, topics: Dict = None, **kwargs):
    ...
    def build_prompt(self, question_data: Dict[str, str|Any]) -> Tuple[str, str]:
        ...
        topics = self.topics.get(qid, []) if self.topics else []
        topic_str = ", ".join(topics) if isinstance(topics, list) else str(topics)
        
        question_with_topic = f"(This question relates to: {topic_str})\n\n{question_data['question']}"
...
if __name__ == "__main__":
    ...
    # ✅ Load generation configs early
    with open("agen.yaml", "r") as f:
        gen_kwargs = yaml.safe_load(f)
    
    # Load the topics from new_topics.json
    with open("/jupyter-tutorial/AAIPL_134_199_198_15/assets/topics_example_shravan.json", 'r') as f:
        new_topics = json.load(f)

    # Pass the topics to the AnsweringAgent (you might need to adjust how the agent uses this data)
    agent = AnsweringAgent(select_prompt1=SELECT_PROMPT1, topics=new_topics, **gen_kwargs)
